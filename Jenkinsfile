        ...
        stages {
            ...
            stage('Build-Image') {
                steps {      
                    ...
                    sh 'docker build -t sivakumar2606/multiblue:prod .'
                }
            }
            stage('Push to Hub') {
                steps {
                    sh 'docker push sivakumar2606/multiblue:prod'
                }
            } 
            stage('Deploy to Docker-Host') {
                steps {     
                    sh 'docker -H tcp://10.0.0.5:2375 stop myprodweb'
                    sh 'docker -H tcp://10.0.0.5:2375 run --rm -dit --name myprodweb -p 9300:80  sivakumar2606/multiblue:prod' 
                }
            }
